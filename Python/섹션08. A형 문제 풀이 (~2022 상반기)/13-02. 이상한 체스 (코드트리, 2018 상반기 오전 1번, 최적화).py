...
chess5 = [CHESS(0, 0, 0) for _ in range(8 + 2)]
cidx, cidx5 = 0, 0
...
def input_data():
    ...
    for r in range(1, N + 1):
        for c in range(1, M + 1):
            ...
            if MAP[r][c] == 5:                
                chess5[cidx5] = CHESS(r, c, MAP[r][c])
                cidx5 += 1
            else:
...
def check_area(map, sr, sc, direction):
    r, c = sr, sc
    while True:
        r += dr[direction]
        c += dc[direction]
...
def simulate():
    ...
    for i in range(cidx):
        ...
        if chessNumber == 1:
            check_area(tmpMAP, sr, sc, direction)
        elif chessNumber == 2:
            inverse = direction + 2
            if inverse > 3: inverse -= 4
            
            check_area(tmpMAP, sr, sc, direction)
            check_area(tmpMAP, sr, sc, inverse)
        elif chessNumber == 3:
            next_dir = direction + 1
            if next_dir == 4: next_dir = 0
            
            check_area(tmpMAP, sr, sc, direction)
            check_area(tmpMAP, sr, sc, next_dir)
        elif chessNumber == 4:
            for i in range(4):
                if i == direction: continue
                
                check_area(tmpMAP, sr, sc, i)
...
for tc in range(T): 
    ...
    for i in range(cidx5):
        sr = chess5[i].r
        sc = chess5[i].c
        
        check_area(MAP, sr, sc, 0)
        check_area(MAP, sr, sc, 1)
        check_area(MAP, sr, sc, 2)
        check_area(MAP, sr, sc, 3)
