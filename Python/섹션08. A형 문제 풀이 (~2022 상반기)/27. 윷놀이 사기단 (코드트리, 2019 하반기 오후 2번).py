...
move = [0] * MAX
next = [[0] * 6 for _ in range(MAX_BOARD)]
score = [0] * MAX_BOARD

num_of_cases = [0] * MAX

def input_data():
    global move, score, next

    move[:10] = list(map(int, input().split()))
...
def simulate():
    horse_pos = [0] * 5
    board = [False] * MAX_BOARD
    
    sum = 0
    for i in range(10):
        horse_number = num_of_cases[i]
        move_count = move[i]

        current_pos = horse_pos[horse_number]
        next_pos = next[current_pos][move_count]

        if next_pos != DESTINATION and board[next_pos] == True: return 0

        horse_pos[horse_number] = next_pos
        
        board[current_pos] = False
        board[next_pos] = True
